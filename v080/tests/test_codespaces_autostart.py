...
def test_v080_start_uses_single_quality_runtime_and_prompt_workspace() -> None:
    ...
    assert 'EXPECTED_PROMPT_ADAPTER="1.0.1"' in start
    ...
    assert 'Nano Banana Prompt Adapter' in start
    assert 'Prompt editor: final provider prompt is editable and versioned' in start
    assert 'Project deletion: active except during running generation' in start
    assert 'skill_engine' in init
    assert 'provider_retry' in init
    assert 'nano_banana_prompt_adapter' in init
