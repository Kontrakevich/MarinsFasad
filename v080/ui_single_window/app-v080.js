(() => {
  ...
  const STAGE_LABELS = {source:'Исходник', geometry:'Геометрия', environment:'Окружение', final:'Final', branding:'Вывеска'};
  let current = null;
  let projectsCache = [];
  ...
  function assetUrl(key) {
    return current?.assets?.[key] ? `/api/projects/${current.id}/assets/${key}?t=${Date.now()}` : '';
  }
  ...
  async function loadHealth() {
    try {
      ...
      node.innerHTML = `<i></i>${data.runtime === 'standalone-v080' ? 'SYSTEM ONLINE' : 'RUNTIME MISMATCH'}`;
    } catch (error) {
  ...
  async function loadProjects(selectFirst = false) {
    ...
    projectsCache.forEach(project => {
      ...
      const canvas = project.master_canvas ? `${project.master_canvas.width} × ${project.master_canvas.height}` : 'Master не загружен';
  ...
  async function selectProject(id) {
    ...
    currentView = currentStage === 'geometry' && current.assets?.source_master ? 'grid' : 'original';
    selectedAssetKey = null;
  ...
  function resetGeometryRuntime() {
    geo.image = null; geo.corners = []; geo.drag = -1; geo.history = []; geo.future = []; geo.projectId = null; geo.sourcePath = null;
  }
  ...
  function renderAll() {
    ...
    $('header-master').textContent = current.master_canvas ? `${current.master_canvas.width} × ${current.master_canvas.height}` : '—';
    $('source-status').textContent = current.master_canvas ? `Оригинал ${current.master_canvas.width} × ${current.master_canvas.height}. Production policy: original resolution.` : 'Файл не загружен.';
  ...
  function stageResultKey(stage = currentStage) {
    if (selectedAssetKey) return selectedAssetKey;
    if (stage === 'source') return current?.assets?.source_master ? 'source_master' : null;
    if (stage === 'geometry') return current?.assets?.geometry_candidate ? 'geometry_candidate' : (current?.assets?.source_master ? 'source_master' : null);
    if (stage === 'environment') return current?.assets?.environment_candidate ? 'environment_candidate' : (current?.assets?.geometry_candidate ? 'geometry_candidate' : 'source_master');
    if (stage === 'final') return current?.assets?.environment_candidate ? 'environment_candidate' : (current?.assets?.geometry_candidate ? 'geometry_candidate' : 'source_master');
    if (stage === 'branding') return current?.assets?.branding_candidate ? 'branding_candidate' : (current?.assets?.environment_candidate ? 'environment_candidate' : 'source_master');
    return 'source_master';
  }

  function renderStageActions() {
    ['source','geometry','environment','final','branding'].forEach(stage => $(`${stage}-actions`).classList.toggle('hidden', stage !== currentStage));
    const titles = {
      ...
      environment:['AI ENVIRONMENT','Заполните outpaint-зоны без изменения архитектуры'],
  ...
  function setCurrentStage(stage) {
    currentStage = stage;
    selectedAssetKey = null;
    currentView = stage === 'geometry' && current.assets?.source_master ? 'grid' : (stage === 'source' ? 'original' : 'result');
    renderAll();
  ...
  function setView(view) {
    currentView = view;
    document.querySelectorAll('.mode-tabs button').forEach(button => button.classList.toggle('active', button.dataset.view === view));
    renderView();
  }

  function hideViewerPanels() {
    ['source-panel','single-view','grid-view','overlay-view','split-view'].forEach(id => $(id).classList.add('hidden'));
  }

  function renderView() {
    if (!current) return;
    document.querySelectorAll('.mode-tabs button').forEach(button => button.classList.toggle('active', button.dataset.view === currentView));
    hideViewerPanels();
    ...
    const originalUrl = assetUrl('source_master');
    const resultKey = stageResultKey();
    const resultUrl = resultKey ? assetUrl(resultKey) : originalUrl;

    if (currentView === 'grid' && currentStage === 'geometry') {
      $('grid-view').classList.remove('hidden');
      loadGeometryImage();
    } else if (currentView === 'overlay') {
      $('overlay-view').classList.remove('hidden');
      $('overlay-base').src = originalUrl;
      $('overlay-result').src = resultUrl;
      $('overlay-result').style.opacity = String(Number($('overlay-range').value) / 100);
    } else if (currentView === 'split') {
      $('split-view').classList.remove('hidden');
      $('split-before').src = originalUrl;
      $('split-after').src = resultUrl;
    } else {
      $('single-view').classList.remove('hidden');
      $('single-image').src = currentView === 'original' ? originalUrl : resultUrl;
      applyZoom();
    }
  }
  ...
  async function uploadSource() {
    ...
    currentStage = 'geometry'; currentView = 'grid'; selectedAssetKey = null;
    resetGeometryRuntime();
    renderAll();
    await loadProjects();
  }

  // Exact Perspective Grid behaviour from v0.7.
  function defaultCorners(image) {
    const x = image.naturalWidth * .12;
    const y = image.naturalHeight * .12;
    return [{x,y},{x:image.naturalWidth-x,y},{x:image.naturalWidth-x,y:image.naturalHeight-y},{x,y:image.naturalHeight-y}];
  }

  function loadGeometryImage() {
    const sourcePath = current.assets?.source_master;
    if (!sourcePath) return;
    if (geo.projectId === current.id && geo.sourcePath === sourcePath && geo.image) { resizeGeometry(); return; }
    ...
    image.onload = () => {
      ...
      geo.corners = Array.isArray(saved) && saved.length === 4 ? saved.map(point => ({x:+point.x,y:+point.y})) : defaultCorners(image);
      geo.history = [];
      geo.future = [];
      resizeGeometry();
    };
  ...
  function drawGeometry() {
    ...
    for (let i=0;i<=8;i++) {
      const u=i/8; let point=bilinear(u,0); context.beginPath(); context.moveTo(point.x,point.y);
      for (let step=1;step<=30;step++) { point=bilinear(u,step/30); context.lineTo(point.x,point.y); }
      context.stroke();
    }
    for (let i=0;i<=6;i++) {
      const v=i/6; let point=bilinear(0,v); context.beginPath(); context.moveTo(point.x,point.y);
      for (let step=1;step<=30;step++) { point=bilinear(step/30,v); context.lineTo(point.x,point.y); }
      context.stroke();
    }
    context.strokeStyle='#fff'; context.lineWidth=Math.max(3,canvas.width/700); context.setLineDash([16,10]);
    context.beginPath(); context.moveTo(geo.corners[0].x,geo.corners[0].y); geo.corners.slice(1).forEach(point=>context.lineTo(point.x,point.y)); context.closePath(); context.stroke(); context.setLineDash([]);
    geo.corners.forEach((point,index) => {
      context.beginPath(); context.arc(point.x,point.y,Math.max(12,canvas.width/140),0,Math.PI*2); context.fillStyle='#008a90'; context.fill(); context.strokeStyle='#fff'; context.stroke();
      context.fillStyle='#fff'; context.font=`${Math.max(18,canvas.width/70)}px Arial`; context.textAlign='center'; context.textBaseline='middle'; context.fillText(String(index+1),point.x,point.y);
    });
  }

  function geometryPosition(event) {
    const canvas = $('geometry-canvas'); const rect = canvas.getBoundingClientRect();
    return {x:(event.clientX-rect.left)*canvas.width/rect.width, y:(event.clientY-rect.top)*canvas.height/rect.height};
  }

  function geometryHit(point) {
    const canvas=$('geometry-canvas'), rect=canvas.getBoundingClientRect(), radius=30*canvas.width/rect.width;
    let index=-1,best=Infinity;
    geo.corners.forEach((corner,i)=>{const distance=Math.hypot(point.x-corner.x,point.y-corner.y);if(distance<radius&&distance<best){index=i;best=distance;}});
    return index;
  }
  ...
  async function applyGeometry() {
    ...
    selectedAssetKey = 'geometry_candidate'; currentView = 'split'; renderAll(); await Promise.all([loadHistory(),loadProjects()]);
  }

  async function approveGeometry() {
    current = await api(`/api/projects/${current.id}/geometry/approve`, {method:'POST'});
    currentStage='environment'; currentView='result'; selectedAssetKey=null; renderAll(); await Promise.all([loadHistory(),loadProjects()]);
  }
  ...
  async function addComment() {
    ...
    current = await api(`/api/projects/${current.id}/comments/${currentStage}`, {method:'POST', body:formData({comment})});
    $('comment-input').value=''; renderAll(); await Promise.all([loadHistory(),loadProjects()]);
  }
  ...
  async function generateEnvironment() {
    ...
    currentView='result'; selectedAssetKey='environment_candidate'; renderAll(); await Promise.all([loadHistory(),loadProjects()]);
  }

  async function approveEnvironment() {
    current = await api(`/api/projects/${current.id}/environment/approve`, {method:'POST'});
    currentStage='final'; currentView='result'; selectedAssetKey=null; renderAll(); await Promise.all([loadHistory(),loadProjects()]);
  }
  ...
  function renderQuality(report) {
    ...
    output.innerHTML = entries.map(([key,value]) => {
      ...
      return `<div class="quality-row"><span>${escapeHtml(key)}</span><strong class="${className}">${escapeHtml(typeof value === 'object' ? JSON.stringify(value) : value)}</strong></div>`;
    }).join('') || 'Нет данных.';
  }
  ...
  function formatTime(value) {
    try { return new Date(value).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'}); } catch (_) { return value || ''; }
  }
  ...
  function updateCommentStatus() {
    ...
    $('comment-status').textContent = comments.length ? `${comments.length} комментариев включено в prompt` : 'Комментариев нет';
  }

  function renderCandidates() {
    const definitions = [
      ['source_preview','Original'],['geometry_preview','Geometry'],['environment_preview','Environment'],['branding_preview','Branding']
    ];
    const target=$('candidate-list'); target.innerHTML='';
    definitions.filter(([key])=>current.assets?.[key]).forEach(([key,label])=>{
      ...
      const button=document.createElement('button'); button.className=`candidate-card${selectedAssetKey===productionKey?' active':''}`;
      button.innerHTML=`<img src="${assetUrl(key)}" alt="${escapeHtml(label)}"><span>${escapeHtml(label)}</span>`;
      button.onclick=()=>{selectedAssetKey=productionKey;currentView='result';renderAll();}; target.appendChild(button);
    });
    if (!target.children.length) target.innerHTML='<div class="status-line">Кандидатов пока нет.</div>';
  }

  function openInspectorTab(name) {
    document.querySelectorAll('.inspector-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.tab===name));
    document.querySelectorAll('.inspector-panel').forEach(panel=>panel.classList.toggle('active',panel.dataset.panel===name));
  }

  function openBottomTab(name) {
    document.querySelectorAll('.bottom-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.bottom===name));
    document.querySelectorAll('.bottom-content').forEach(panel=>panel.classList.toggle('active',panel.dataset.bottomPanel===name));
  }

  const canvas=$('geometry-canvas');
  canvas.onpointerdown=event=>{const index=geometryHit(geometryPosition(event));if(index<0)return;geo.history.push(geo.corners.map(point=>({...point})));geo.future=[];geo.drag=index;canvas.setPointerCapture(event.pointerId);};
  canvas.onpointermove=event=>{if(geo.drag<0)return;const point=geometryPosition(event);geo.corners[geo.drag]={x:Math.max(0,Math.min(canvas.width,point.x)),y:Math.max(0,Math.min(canvas.height,point.y))};drawGeometry();};
  canvas.onpointerup=event=>{geo.drag=-1;try{canvas.releasePointerCapture(event.pointerId);}catch(_){}};
  canvas.ondblclick=resetGeometry;

  document.querySelectorAll('#pipeline button').forEach(button=>button.onclick=()=>setCurrentStage(button.dataset.stage));
  document.querySelectorAll('.mode-tabs button').forEach(button=>button.onclick=()=>setView(button.dataset.view));
  document.querySelectorAll('.inspector-tabs button').forEach(button=>button.onclick=()=>openInspectorTab(button.dataset.tab));
  document.querySelectorAll('.bottom-tabs button').forEach(button=>button.onclick=()=>openBottomTab(button.dataset.bottom));

  $('new-project').onclick=createProject; $('new-project-bottom').onclick=createProject; $('empty-new-project').onclick=createProject;
  $('upload-source').onclick=()=>busy($('upload-source'),uploadSource);
  $('source-replace').onclick=()=>{setCurrentStage('source');$('source-file').click();};
  $('geometry-reset').onclick=resetGeometry; $('geometry-undo').onclick=undoGeometry; $('geometry-redo').onclick=redoGeometry;
  ...
  $('overlay-range').oninput=()=>{$('overlay-result').style.opacity=String(Number($('overlay-range').value)/100);};
  $('fit-view').onclick=()=>{fitMode=true;zoomFactor=1;applyZoom();};
  $('actual-view').onclick=()=>{fitMode=false;zoomFactor=1;applyZoom();};
  $('zoom-in').onclick=()=>{fitMode=false;zoomFactor=Math.min(8,zoomFactor+.25);applyZoom();};
  $('zoom-out').onclick=()=>{fitMode=false;zoomFactor=Math.max(.1,zoomFactor-.25);applyZoom();};
  $('fullscreen-view').onclick=()=>{$('viewer').requestFullscreen?.();};
  $('error-close').onclick=()=>$('error-modal').classList.add('hidden');
  $('error-modal').onclick=event=>{if(event.target===$('error-modal'))$('error-modal').classList.add('hidden');};
  window.addEventListener('resize',()=>{if(currentView==='grid')resizeGeometry();});
