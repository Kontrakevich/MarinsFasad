...
const apiFromQuery = (pageParams.get("api") || "").trim();
if (apiFromQuery) {
  localStorage.setItem("marinsApiBase", apiFromQuery.replace(/\/$/, ""));
}
const API_BASE = (apiFromQuery || localStorage.getItem("marinsApiBase") || "").replace(/\/$/, "");
...
function updateHostingNotice() {
  const notice = document.querySelector("#hostingNotice");
  if (!notice) return;

  if (API_BASE) {
    notice.textContent = `GitHub Pages · API: ${API_BASE}`;
    notice.classList.add("connected");
  } else {
    notice.textContent = "GitHub Pages · интерфейс опубликован · подключите backend через ?api=https://ВАШ-API";
  }
}
...
function fileUrl(key) {
  ...
  return (
    `${API_BASE}/api/projects/${current.id}/file/${key}` +
    `?t=${Date.now()}`
  );
}
...
async function reloadProject() {
  ...
  await updateHostingNotice();
refreshProjectList().catch(error => {
  console.error(error);
  const notice = document.querySelector("#hostingNotice");
  if (notice && !API_BASE) notice.classList.add("warning");
});
}
